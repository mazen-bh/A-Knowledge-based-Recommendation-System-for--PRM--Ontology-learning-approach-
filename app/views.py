...
# We use cosine-similarity and torch.topk to find the highest 3 scores
    ...
    for score, idx in zip(top_results[0], top_results[1]):
        ...
        if type(df['paths'].iloc[idx].is_instance_of)!= owlready2.util.CallbackList:
           if len(list(df['paths'].iloc[idx].descendants(include_self = False))) !=0:
                ...
                sous_concept=['paths'].iloc[idx].instances()
    
    #for i in range (len(sous_concept)):
        #re.sub(r"out.", "", str(sous_concept[i]))
